...
def solve_FR(coord , coxa , femur , tibia): 
    ...
    gamma_2 = np.arctan2(-np.sqrt(1-D**2),D)
    tetta = -np.arctan2(-coord[2],coord[1])-np.arctan2(np.sqrt(coord[1]**2+coord[2]**2-coxa**2),-coxa)
    alpha = np.arctan2(coord[0],np.sqrt(coord[1]**2+coord[2]**2-coxa**2))-np.arctan2(tibia*np.sin(gamma_2),femur+tibia*np.cos(gamma_2))
    angles = np.array([tetta, alpha+0.25, gamma-2.125 ])
    print(angles,'1')
    return angles
...
def solve_BR(coord , coxa , femur , tibia): 
    ...
    tetta = -np.arctan2(-coord[2],coord[1])-np.arctan2(np.sqrt(coord[1]**2+coord[2]**2-coxa**2),-coxa)
    alpha = np.arctan2(coord[0],np.sqrt(coord[1]**2+coord[2]**2-coxa**2))-np.arctan2(tibia*np.sin(gamma_2),femur+tibia*np.cos(gamma_2))
    angles = np.array([tetta, alpha+.125 , gamma-2.25])
    print(angles,'3')
    return angles
